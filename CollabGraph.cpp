...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    Vertex *artistV = graph.at(name);
    int length = static_cast<int>(artistV->neighbors.size());
    for(int i = 0; i < length; i++){
        result.push_back(artistV->neighbors.at(i).neighbor);
    }
    return result;
}
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
                                            ...
                                            const Artist &dest) const
{
    ...
    if(source.get_name() == dest.get_name()){
        return path;
    } else if(get_predecessor(dest).get_name() == ""){
        return path;
    } else {
        Artist begin = dest;
        while(begin.get_name() != ""){
            path.push(begin);
            begin = get_predecessor(begin);
        }
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * function: add_edges
 * purpose: fills the graph by taking an artists and looking at the discography
    array and adding the corresponding edges
 * parameters: std::list<Artist> &artist
 * returns: none
 */

void CollabGraph::add_edges(std::list<Artist> &artist)
{
    for (Artist a : artist){
        for(Artist b : artist){ 
            if(a.get_name() != b.get_name()){
                string newEdge = a.get_collaboration(b);
                if(newEdge != ""){
                   insert_edge(a, b, newEdge);
                }
            }
        }
...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * function: get_artist
 * purpose: takes in an artists name, which is the key, and returns the Artists
    struct from the corresponding vector
 * parameters: string name
 * returns: none
 */
Artist CollabGraph::get_artist(string name)
{
    return this->graph.at(name)->artist;
}
